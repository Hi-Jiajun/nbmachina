...
**真机实测（2026-09-18，用户截图回传）**：`/nbfc play` 跑了 21.8s、已调度 113 / 发声 113 / 跳过 0，

| 指标 | 实测 | 对照 |
|---|---|---|
| 调度抖动（均） | **0.18 ms** | 20 tps = 50ms / 100 tps = 10ms |
| 调度抖动（最大） | **1.36 ms** | 同上（约 1/37 与 1/7） |
| 音频队列延迟（入队→执行） | **0.01 ms** | 改造前最多 15ms |

同时确认：资源包**未启用**时全链路正常（无损引擎不受影响）；被抢声部 0、八度折叠 0。
顺带一个出片建议：录制时先 `/tick rate 100`，让**灯/粒子**的视觉误差也压到 ±5ms（声音已经是 ~1ms 级，
只需管视觉）。
