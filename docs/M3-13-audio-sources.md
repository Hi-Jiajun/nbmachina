...
**已定稿（2026-09-15，用户拍板）**：接进数据包。第一版 A/B 用户反馈"力度强弱关系我没有听出来"
——核对后确认这是**正确的听感**（口径上只有 ~1dB 变化、且所有音落在同一采样层）；
真正的修法见 `docs/M3-14-dynamics.md`（分位数归一成 `velMidi` 1..127 + 采样层与音量共用一把尺子），
数据包已重生成并装机，无头 e2e 295/295、MSPT 7.2ms。
